...
LOCAL_FILE = "active_ironman_profiles.json"
TARGET_PATH = os.path.join(REPO_PATH, "data", LOCAL_FILE)

with open(LOCAL_FILE, 'w', encoding='utf-8') as f:
    json.dump(active_profiles_per_player, f, indent=2, ensure_ascii=False)

print(f"Data saved {len(active_profiles_per_player)} players to {LOCAL_FILE}")
...
def upload_via_git() -> bool:
    ...
    subprocess.run(["git", "-C", REPO_PATH, "commit", "-m", f"Ironman Data Update - {timestamp}"], check=True)
    subprocess.run(["git", "-C", REPO_PATH, "push"], check=True)

    print("File sent via SSH Deploy Key.")
    return True
